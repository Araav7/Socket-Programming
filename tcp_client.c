...
int main(int argc, char **argv)
{
	...
	bzero(&(ser_addr.sin_zero), 8); //appends zeros at the end to satisfy requirement
  ...
  /*** the whole file is loaded in the buffer. ***/
	...
	while(ci<= lsize)
	{
		if ((lsize+1-ci) <= DATALEN)
			...
			slen = DATALEN; //sending whole file in data units of size DATALEN
		...
		if (ack.num != 1|| ack.len != 0) {
		  ...
		  ci += slen; //go to the next packet (CI refers to the current index and slen refers to to the size of the packet)
		}
